...
client.on('messageCreate', async (message) => {
  ...
  try {
    ...
    if (!inDM && ownerMissingOrBot && !publicInitializedGuilds.has(message.guild.id)) {
      for (const key of commands.keys()) {
        client.db.set(`perm_${key}.${message.guild.id}`, 'public');
      }
  ...
  // For safety, run non-help commands only in guilds
  if (inDM) {
    await message.channel.send('ℹ️ Les commandes doivent être utilisées sur un serveur. Utilisez `+'+ 'help` pour voir la liste ici.');
    return;
  }
